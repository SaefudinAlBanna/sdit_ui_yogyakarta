...
class TanggapanCatatanKhususSiswaController extends GetxController {
  var dataArgumen = Get.arguments;

  TextEditingController tanggapankepalasekolahC = TextEditingController();
  ...
  Future<void> updateTanggapanKepalaSekolah(
    String docIdCatatanSiswa, // ID unik dari dokumen catatansiswa
    String idWaliKelasDariData, // Diambil dari data catatansiswa
    String idGuruBK, // Diambil dari data catatansiswa
    ...
    if (tanggapankepalasekolahC.text.trim().isEmpty) {
    ...
    final String tanggapanBaru = tanggapankepalasekolahC.text;
    ...
    DocumentReference kepalaSekolahRef = firestore
        ...
        .doc(idSekolah)
        .collection('pegawai')
        .doc(idUser) // idKepalaSekolah saat ini
        .collection('tahunajaran')
    ...
    batch.update(kepalaSekolahRef, {'tanggapankepalasekolah': tanggapanBaru});

    // 2. UPDATE PADA COLLECTION WALIKELAS
    // Asumsi struktur path mirip, hanya idPegawai-nya yang beda
    DocumentReference waliKelasRef = firestore
        .collection('Sekolah')
        .doc(idSekolah)
        .collection('pegawai')
        .doc(idWaliKelasDariData) // ID Walikelas dari data catatan
        .collection('tahunajaran')
        .doc(formattedIdTahunAjaran)
        .collection('kelas')
        ...
        .collection('catatansiswa')
        .doc(docIdCatatanSiswa);
    batch.update(waliKelasRef, {'tanggapankepalasekolah': tanggapanBaru});
     ...
     DocumentReference guruBKRef = firestore
        ...
        .collection('catatansiswa')
        .doc(docIdCatatanSiswa);
    batch.update(guruBKRef, {'tanggapankepalasekolah': tanggapanBaru});
    ...
    DocumentReference siswaRef = firestore
        ...
        .collection('catatansiswa')
        .doc(docIdCatatanSiswa);
    batch.update(siswaRef, {'tanggapankepalasekolah': tanggapanBaru});

...
class ImportSiswaExcelController extends GetxController {
  ...
  Future<void> _processExcel() async {
    ...
    try {
      ...
      // --- PERUBAHAN 1: Validasi Header Excel disesuaikan ---
      if (sheet.rows.isEmpty || 
          sheet.rows.first[0]?.value.toString().trim() != 'NISN' ||
          sheet.rows.first[1]?.value.toString().trim() != 'Nama' ||
          sheet.rows.first[2]?.value.toString().trim() != 'SPP') { // Email dihapus dari header
      ...
      for (var i = 1; i < sheet.maxRows; i++) {
        ...
        // --- PERUBAHAN 2: Ekstrak data disesuaikan, email dibuat otomatis ---
        final nisn = row[0]?.value?.toString().trim();
        final nama = row[1]?.value?.toString().trim();
        final spp = num.tryParse(row[2]?.value?.toString().trim() ?? '') ?? 0; // Kolom SPP sekarang di index 2
        ...
        // --- PERUBAHAN 3: Membuat email palsu dari NISN ---
        final String emailPalsu = "$nisn@sditui.com";

        try {
          // --- PERUBAHAN 4: Membuat user di Auth dengan email palsu ---
          UserCredential siswaCredential = await auth.createUserWithEmailAndPassword(
            email: emailPalsu,
            password: 'sditui' // Password default
          );
          
          // --- DIHAPUS: Baris sendEmailVerification() dihapus karena email tidak nyata ---
          // await siswaCredential.user!.sendEmailVerification();
          
          // Login kembali sebagai admin untuk melanjutkan operasi Firestore
          await auth.signInWithEmailAndPassword(email: emailAdmin, password: adminPassword);
          
          // --- PERUBAHAN 5: Simpan ke Firestore menggunakan UID dari Auth sebagai ID dokumen ---
          ...
          await firestore.collection("Sekolah").doc(idSekolah).collection('siswa').doc(nisn).set({
              ...
              "nisn": nisn,
              "nama": nama,
              "email": emailPalsu, // Simpan email palsu untuk referensi
              "spp": spp,
              "wajibGantiPassword": true, // Penanda untuk memaksa ganti password saat login pertama
              "status": "baru",
              "jeniskelamin": null,
              "tanggalLahir": null,
              "alamat": null,
          ...
          successCount.value++;
            ...
            errorCount.value++;
            String errorMsg = e.code == 'email-already-in-use' ? "NISN (email palsu) sudah terdaftar." : "Gagal membuat user.";
            errorDetails.add("Baris ${i + 1} ($nisn): $errorMsg");
            // Login kembali sebagai admin jika pembuatan user gagal agar loop bisa lanjut
            await auth.signInWithEmailAndPassword(email: emailAdmin, password: adminPassword);
        }
      }
  ...
  Future<void> downloadTemplate() async {
    ...
    try {
      ...
      // --- PERUBAHAN 6: Header template diubah ---
      ...
      // --- PERUBAHAN 7: Contoh data di template disesuaikan ---
      sheetObject.appendRow([
        TextCellValue("123456789"),      // Contoh NISN
        TextCellValue("Contoh Nama Siswa"), // Contoh Nama
        IntCellValue(150000)                // Contoh SPP
      ]);
